            ...
            if (VM.SubtitleView.Subtitle_Stream_SelectedItem == "mux")
            {
                try
                {
                    for (var i = 0; i < VM.SubtitleView.Subtitle_ListView_Items.Count; i++)
                    {
                        ...
                        {
                            metadata += "-metadata:s:s:" + i + " title=" + MainWindow.WrapWithQuotes(titleList[i]) + "\r\n";
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(languageList[i]) &&
                            languageList[i] != "none")
                        {
                            metadata += "-metadata:s:s:" + i + " language=" + MainWindow.WrapWithQuotes(
                                                                                VM.SubtitleView.Subtitle_Metadata_Language_Items
